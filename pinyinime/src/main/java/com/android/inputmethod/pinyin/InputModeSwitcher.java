...
public class InputModeSwitcher {
    ...
    private static final int MASK_SKB_LAYOUT_HAND_WRITING_TEXT = 0x60000000;
    
    private static final int MASK_SKB_LAYOUT_HAND_WRITING_FORMULA = 0x70000000;
    ...
    public static final int MODE_SKB_HAND_WRITING_TEXT = (MASK_SKB_LAYOUT_HAND_WRITING_TEXT);
    
    public static final int MODE_SKB_HAND_WRITING_FORMULA = (MASK_SKB_LAYOUT_HAND_WRITING_FORMULA);
    ...
    private int mToggleHandWritingText;
    private int mToggleHandWritingFormula;
    ...
    public InputModeSwitcher(PinyinIME imeService) {
        ...
        mToggleHandWritingText= Integer.parseInt(r
                .getString(R.string.toggle_hand_writing_text));
        mToggleHandWritingFormula=Integer.parseInt(r
                .getString(R.string.toggle_hand_writing_formula));
    }
    ...
    public int getSkbLayout() {
        ...
        case MASK_SKB_LAYOUT_HAND_WRITING_TEXT:
        case MASK_SKB_LAYOUT_HAND_WRITING_FORMULA:	
        	return R.xml.skb_hw;
        }
        return 0;
    }
    ...
    public int switchModeForUserKey(int userKey) {
        ...
        if (USERDEF_KEYCODE_LANG_2 == userKey) {
            if (MODE_SKB_CHINESE == mInputMode) {
                newInputMode = MODE_SKB_HAND_WRITING_TEXT;
                ...
                newInputMode = MODE_SKB_CHINESE;
            }else if(MODE_SKB_HAND_WRITING_TEXT == mInputMode){
            	newInputMode = MODE_SKB_HAND_WRITING_FORMULA;
            }else if(MODE_SKB_HAND_WRITING_FORMULA == mInputMode){
            ...
            if (MODE_SKB_CHINESE == mInputMode) {
            	...
            	if(MODE_SKB_SYMBOL1_CN != mPreviousInputMode
                        && MODE_SKB_SYMBOL2_CN != mPreviousInputMode
                        &&MODE_SKB_SYMBOL1_EN != mPreviousInputMode
                        && MODE_SKB_SYMBOL2_EN != mPreviousInputMode
                        ){
            		newInputMode = mPreviousInputMode;
            	}
            	else{
            		newInputMode = MODE_SKB_ENGLISH_LOWER;
            	}
                ...
                newInputMode = MODE_SKB_SYMBOL1_CN;
            }else if(MODE_SKB_HAND_WRITING_TEXT == mInputMode||MODE_SKB_HAND_WRITING_FORMULA== mInputMode){
            	newInputMode = MODE_SKB_SYMBOL1_CN;
            }
    ...
    public int requestInputWithHkb(EditorInfo editorInfo) {
        ...
        if (english) {
            ...
            // mode.
            if ((mRecentLauageInputMode & MASK_LANGUAGE) == MASK_LANGUAGE_EN) {
                newInputMode = MODE_HKB_ENGLISH;
            } else {
    ...
    public int requestInputWithSkb(EditorInfo editorInfo) {
        ...
        case EditorInfo.TYPE_CLASS_TEXT:
            ...
            if (v == EditorInfo.TYPE_TEXT_VARIATION_EMAIL_ADDRESS
                ...
                // If the application request English mode, we switch to it.
                newInputMode = MODE_SKB_ENGLISH_LOWER;
            }else if(v == EditorInfo.TYPE_TEXT_VARIATION_LONG_MESSAGE){
            	newInputMode = MODE_SKB_CHINESE;
            }else {
                ...
                if (0 == skbLayout) {
                    if ((mInputMode & MASK_LANGUAGE) == MASK_LANGUAGE_EN) {
                    	newInputMode = MODE_SKB_ENGLISH_LOWER;
                    } else {      
                    	if(mPreviousInputMode==MODE_SKB_HAND_WRITING_TEXT){
                    		newInputMode = MODE_SKB_HAND_WRITING_TEXT;
                    	}
                    	else if(mPreviousInputMode==MODE_SKB_HAND_WRITING_FORMULA){
                    		newInputMode = MODE_SKB_HAND_WRITING_FORMULA;
                    	}
                    	else{
                    		newInputMode = MODE_SKB_CHINESE;
    ...
    public boolean isHandWritingWithSkb() {
        int skbLayout = (mInputMode & MASK_SKB_LAYOUT);
        if (MASK_SKB_LAYOUT_HAND_WRITING_TEXT == skbLayout 
        		||MASK_SKB_LAYOUT_HAND_WRITING_FORMULA == skbLayout) {
        		return true;
        }
        return false;
    }
    
    public boolean isHandWritingFormulaWithSkb() {
        int skbLayout = (mInputMode & MASK_SKB_LAYOUT);
        if (MASK_SKB_LAYOUT_HAND_WRITING_FORMULA == skbLayout) {
        		return true;
        }
        return false;
    }
    ...
    private void prepareToggleStates(boolean needSkb) {
        ...
        if (MASK_SKB_LAYOUT_PHONE != layout) {
        	...
        	if(MASK_SKB_LAYOUT_HAND_WRITING_TEXT == layout){
        		states[statesNum] = mToggleHandWritingText;
        		statesNum++;
        	}
        	
        	if(MASK_SKB_LAYOUT_HAND_WRITING_FORMULA == layout){
        		states[statesNum] = mToggleHandWritingFormula;
        		statesNum++;
        	}
