...
public class CandidateView extends View {
    ...
    public CandidateView(Context context, AttributeSet attrs) {
        ...
        mActiveCellDrawable = r.getDrawable(R.color.candidate_hl_bg_color);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(pSize==1){
        	String cand = mDecInfo.mCandidatesList.get(pStart);
            if(cand.length()>=6&&cand.startsWith("$${")&&cand.endsWith("}$$") ){
    			FormulaDraw f = new FormulaDraw(getContext());
    			try{
	    			...
	    			int offsetX=(getHeight()-h)/2;
	    			int offsetY=0;
	    			if(w<=40) offsetY=20;
	    			Rect dst=new Rect((int) mPaddingLeft+offsetY, mPaddingTop+offsetX, mPaddingLeft+offsetY+w, mPaddingTop+offsetX+h);
	                
	    			Drawable ActiveDrawable=new ColorDrawable(Color.WHITE);
	    			ActiveDrawable.setBounds((int) mPaddingLeft,
	                        (int) mPaddingTop, (int) mPaddingLeft+w+2*offsetY,
	                        (int) mPaddingTop+getHeight());
	    			ActiveDrawable.draw(canvas);
	    			canvas.drawBitmap(bitmap, null, dst,
	                        mCandidatesPaint);
	                
	                if (mCandRects.size() < pSize) mCandRects.add(new RectF());
	                mCandRects.elementAt(0).set(dst);
    ...
    private float drawVerticalSeparator(Canvas canvas, float xPos) {
    	...
    	int top=mPaddingTop+(getHeight()-mSeparatorDrawable.getIntrinsicHeight())/2;
    	int bottom=top+mSeparatorDrawable.getIntrinsicHeight();
        mSeparatorDrawable.setBounds((int) xPos, top, 
        		(int) xPos+mSeparatorDrawable.getIntrinsicWidth(), bottom);
