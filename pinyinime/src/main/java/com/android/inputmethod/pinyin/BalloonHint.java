...
import com.hanvon.test.FormulaDraw;
...
public class BalloonHint extends PopupWindow {
    ...
    public void setBalloonConfig(String label, float textSize,
    	...
    	boolean done=false;
    	Drawable icon=null;
        if(cand.length()>=6&&cand.startsWith("$${")&&cand.endsWith("}$$") ){
			FormulaDraw f = new FormulaDraw(BalloonHint.this.getContentView().getContext());
			try{
				String result=cand.substring(3, cand.length()-3);
			  			
    			f.draw(result);
    			String path = f.getResultFileName() + f.tempFileName;
    			Bitmap bitmap=new BitmapDrawable(path).getBitmap();
    			icon=new BitmapDrawable(bitmap);
    			setBalloonConfig(icon,  width,  height);
                done=true;
			}catch (Exception e) {
